...
- 顶部“导入 PID”可从任意文段自动提取 PID，按 `REQUEST_MAX_PARALLEL` 并发抓取帖子与评论预览并写入本地缓存。
...
PID 扫描是并发的：任务会批量提交到线程池，`REQUEST_MAX_PARALLEL` 控制同时在飞的请求数，`REQUEST_MAX_REQUESTS_PER_SECOND` 控制提交速率。单帖结果和 getpost 自带评论预览会写入 `data/pid_post_cache/`；未找到的 PID 也会短时间缓存，避免反复扫删除空洞。
...
QUICK_QA_SEARCH_RESULTS_PER_CALL = 25
QUICK_QA_CONTEXT_POST_LIMIT = 60
QUICK_QA_COMMENT_FETCH_POSTS = 5
...
QUICK_QA_FOCUSED_SEARCH_HINT_MAX = 5
...
DEEP_RESEARCH_COMMENT_FETCH_POSTS = 12
...
# Direct tool-call defaults outside a quick/deep profile.
DEFAULT_SEARCH_RESULTS_PER_CALL = 60
DEFAULT_COMMENT_FETCH_POSTS = 5
...
THOROUGH_SEARCH_MAX_RESULTS_PER_KEYWORD = 8000
THOROUGH_SEARCH_MAX_CONTEXT_POSTS = 40
THOROUGH_SEARCH_CONTEXT_COMMENTS_PER_POST = 10
```

`agent.py` 现在直接读取这些字段，不再兼容旧的 `MAX_SEARCH_RESULTS`、`MAX_CONTEXT_POSTS`、`MAX_COMMENT_FETCH_POSTS`、`MAX_COMMENTS_PER_POST` 或全局 `BROAD_SEARCH_*` / `FOCUSED_SEARCH_*`。如果使用 `config_private.py`，需要和 `config.py` 保持同一套字段。
